#!/usr/bin/env python3
"""Sube admin-web/ a public_html del subdominio admin."""
...
def main() -> int:
    ...
    pub = cfg.get("DTUNNEL_ADMIN_PATH_PUBLIC")
    if not pub:
        print("ERROR: Falta DTUNNEL_ADMIN_PATH_PUBLIC en secretos/.env.dtunnel", file=sys.stderr)
        return 1
    if not ADMIN_ROOT.is_dir():
        print(f"ERROR: No existe {ADMIN_ROOT}", file=sys.stderr)
        return 1

    user = cfg["DTUNNEL_USER"]
    buf = BytesIO()
    with tarfile.open(fileobj=buf, mode="w:gz") as tar:
        for f in ADMIN_ROOT.rglob("*"):
            if f.is_file():
                tar.add(f, arcname=f.relative_to(ADMIN_ROOT).as_posix())
