...
app.post('/auth/change-password', authRequired, loginLimiter, (req, res) => {
  ...
  if (!currentPassword || !password || password.length < 8) {
    return res.status(400).json({ error: 'Contraseña actual y nueva (mín. 8 caracteres) requeridas' });
  }
  if (!verifyPassword(req.dbUser, currentPassword)) {
    return res.status(401).json({ error: 'Contraseña actual incorrecta' });
  }
  updateUserPassword(req.user.userId, password);
  res.json({ ok: true });
});
